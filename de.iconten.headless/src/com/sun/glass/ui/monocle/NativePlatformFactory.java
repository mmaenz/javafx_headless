...
public abstract class NativePlatformFactory {
	...
	/**
	 * Creates a NativePlatform. This is only called if matches() was previously
	...
	/**
	 * Obtains a NativePlatform that matches the platform on which we are
	 * running.
	 *
	 ...
	 * .platform can be overridden to select a specific platform. For example,
	 * running with -Dmonocle.platform=Dispman, Linux selects the NativePlatform
	 * that works with the dispmanx libraries on the Raspberry Pi, but falls
	 * back to a generic software-rendered Linux framebuffer implementation if
	 * we are not running on a device with dispmanx libraries.
	 *
	 * @return a new NativePlatform
	 */
	public static synchronized NativePlatform getNativePlatform() {
		if (platform == null) {
			final String platformFactoryProperty = AccessController.doPrivileged(
					(PrivilegedAction<String>) () -> System.getProperty("monocle.platform", "MX6,OMAP,Dispman,Android,X11,Linux,Headless"));
			final String[] platformFactories = platformFactoryProperty.split(",");
			for (int i = 0; i < platformFactories.length; i++) {
				final String factoryName = platformFactories[i].trim();
				String factoryClassName;
				if (factoryName.contains(".")) {
					factoryClassName = factoryName;
				} else {
					factoryClassName = "de.iconten.glass.ui.monocle." + factoryName + "PlatformFactory";
				}
				...
				try {
					final NativePlatformFactory npf = (NativePlatformFactory) Class.forName(factoryClassName).newInstance();
					if (npf.matches() && npf.getMajorVersion() == majorVersion && npf.getMinorVersion() == minorVersion) {
