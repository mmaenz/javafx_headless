...
final class MonocleWindow extends Window {
	...
	/**
	 *
	 * w/h is the total window width/height including all its decorations (e.g.
	 * title bar). cw/ch is the "client", or interior width/height. Negative
	 * values for w/h/cw/ch are treated as "not set". For example: setBounds(x,
	 * y, xSet, ySet, 800, 600, -1, -1) will make the window 800x600 pixels. The
	 * client area available for drawing will be smaller, e.g. 792x580 - it
	 * depends on the window decorations on different platforms. setBounds(x, y,
	 * xSet, ySet, -1, -1, 800, 600) will make the window client size to be
	 * 800x600 pixels. The area for drawing (FX scene size) will be exactly
	 * 800x600, but the total window size including decorations will be slightly
	 * bigger. For undecorated windows w/h and cw/ch are obviously the same.
	 *
	 * As this is a void function the native code should trigger an event to
	 * notify the system on actual change
	 *
	 */
	@Override
	protected void _setBounds(long nativeWindowPointer, int x, int y, boolean xSet, boolean ySet, int w, int h, int cw, int ch, float xGravity,
			float yGravity) {
		int width;
		int height;
	...
	private void notifyResizeAndMove(int x, int y, int width, int height) {
		final MonocleView view = (MonocleView) getView();
	...
	protected boolean _maximize(long nativeWindowPointer, boolean maximize, boolean wasMaximized) {
		final NativeScreen screen = NativePlatformFactory.getNativePlatform().getScreen();
		int x = getX();
		int y = getY();
		int width = getWidth();
		int height = getHeight();
	...
	@Override
	protected void _requestInput(long ptr, String text, int type, double width, double height, double Mxx, double Mxy, double Mxz, double Mxt,
			double Myx, double Myy, double Myz, double Myt, double Mzx, double Mzy, double Mzz, double Mzt) {
