...
class AppThread {
	...
	static <T> T exec(final boolean pauseAfterExec, final StatusCode statusCode, final long timeout, final Sync<T> action) {
		try {
			if (Platform.isFxApplicationThread()) {
				return action.perform();
			}
			final Runner<T> runner = new Runner<>(action, statusCode);
			...
			synchronized (runner.done) {
				if (!runner.done.get()) {
					...
					if (!runner.done.get()) {
						...
						//throw new Exception(new StringBuilder().append("Timeout of ").append(timeout).append("ms reached.").toString());
					}
				}
