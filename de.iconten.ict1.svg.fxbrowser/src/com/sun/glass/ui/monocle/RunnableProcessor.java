...
/**
 * RunnableProcessor provides control over nested run loops in the JavaFX event
 * queue.
 */
class RunnableProcessor implements Runnable {
	...
	private final LinkedList<RunLoopControl> activeRunLoops = new LinkedList<>();
	...
	/**
	 * Posts a Runnable to the JavaFX event queue and waits for the Runnable to
	 * complete.
	 *
	 * @param r
	 *            an action to be performed on the JavaFX application thread.
	...
	void leaveNestedEventLoop(Object retValue) {
		...
		final RunLoopControl current = activeRunLoops.pop();
		assert current != null;
	...
	void shutdown() {
		synchronized (queue) {
			...
			while (!activeRunLoops.isEmpty()) {
				final RunLoopControl control = activeRunLoops.pop();
