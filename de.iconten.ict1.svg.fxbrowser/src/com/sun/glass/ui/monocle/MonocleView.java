...
final class MonocleView extends View {
	...
	private final boolean resetCursorVisibility = false;
	...
	protected void _uploadPixels(long nativeViewPtr, Pixels pixels) {
		if (getWindow() != null) {
			...
			screen.uploadPixels(pixels.getPixels(), x + window.getX(), y + window.getY(), pixels.getWidth(), pixels.getHeight(), window.getAlpha());
		}
	}
	...
	@Override
	protected void notifyScroll(int x, int y, int xAbs, int yAbs, double deltaX, double deltaY, int modifiers, int lines, int chars, int defaultLines,
	...
	protected boolean _enterFullscreen(long ptr, boolean animate, boolean keepRatio, boolean hideCursor) {
		final MonocleWindowManager wm = MonocleWindowManager.getInstance();
		final MonocleWindow focusedWindow = wm.getFocusedWindow();
	...
	public String toString() {
		return "MonocleView[" + x + "," + y + "+" + getWidth() + "x" + getHeight() + "]";
	}
	...
	/**
	 * Assuming this is used to unlock the surface after painting is done
	 */
	@Override
